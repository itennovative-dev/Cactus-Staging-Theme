...
(function () {
    ...
    trapFocus: (container, elementToFocus = container) => {
      ...
      FoxTheme.a11y.trapFocusHandlers.focusin = (event) => {
        ...
        document.addEventListener(
          "keydown",
          FoxTheme.a11y.trapFocusHandlers.keydown
        );
      };
      ...
      document.addEventListener(
        "focusout",
        FoxTheme.a11y.trapFocusHandlers.focusout
      );
      document.addEventListener(
        "focusin",
        FoxTheme.a11y.trapFocusHandlers.focusin
      );
  ...
  FoxTheme.utils = {
    ...
    queryDomNodes: (selectors = {}, context = document) => {
      const domNodes = Object.entries(selectors).reduce(
        (acc, [name, selector]) => {
  ...
  FoxTheme.pubsub = {
    ...
    subscribe: (eventName, callback) => {
      ...
      return function unsubscribe() {
        FoxTheme.pubsub.subscribers[eventName] = FoxTheme.pubsub.subscribers[
          eventName
        ].filter((cb) => {
          return cb !== callback;
        });
    ...
    switch (name) {
      case "open":
        ...
        if (
          ...
          if (
            this.requiresBodyAppended &&
            this.parentElement !== document.body
          ) {
            this.parentElementBeforeAppend = this.parentElement;
            document.body.append(this);
          }
          const handleShowTransitionPromise =
            this.handleShowTransition(newValue !== "immediate") ||
            Promise.resolve();
          handleShowTransitionPromise.then(() => {
            this.handleAfterShow();
            this.dispatchEvent(
              new CustomEvent(this.events.handleAfterShow, { bubbles: true })
            );
          });
        } else if (oldValue !== null && newValue === null) {
          this.setAttribute("inert", "");

          const handleHideTransitionPromise =
            this.handleHideTransition() || Promise.resolve();
          handleHideTransitionPromise.then(() => {
            ...
            this.dispatchEvent(
              new CustomEvent(this.events.handleAfterHide, { bubbles: true })
            );
  ...
  handleAfterHide() {
    setTimeout(() => {
      ...
      if (this.isLockingNeeded) {
        ...
        const currentModalCount =
          clearModalComponentCount.get(ModalComponent) - 1;
    ...
    if (isOpen) {
      ...
      await FoxTheme.Motion.timeline([
        [
          ...
          {
            height: [
              `${this.summaryElement.clientHeight + 1}px`,
...
class AccordionGroup extends AccordionDetails {
  ...
  handleAfterToggle() {
    if (this.isOpen) {
      ...
      const siblingsAccordions = parent.querySelectorAll(
        'details[is="accordion-group"]'
      );
...
class ProgressBar extends HTMLElement {
  ...
  setPercentage(val1, val2) {
    this.style.setProperty(
      "--percent",
      `${(parseInt(val1) / parseInt(val2)) * 100}%`
    );
  }
}
  ...
  constructor() {
    ...
    if (this.posterElement) {
      this.posterElement.addEventListener(
        "click",
        this.handlePosterClick.bind(this)
      );
    }
    ...
    if (name === "playing") {
      if (oldValue === null && newValue === "") {
        return this.dispatchEvent(
          new CustomEvent("video:play", { bubbles: true })
        );
      }

      if (newValue === null) {
        return this.dispatchEvent(
          new CustomEvent("video:pause", { bubbles: true })
        );
      }
    }
  }
...
class LocalizationForm extends HTMLElement {
  ...
  handleDropdownPos() {
    ...
    if (window.innerWidth - offsetButton < 220) {
      this.elements.button.nextElementSibling &&
        this.elements.button.nextElementSibling.classList.add(
  ...
  closeSelector(event) {
    const shouldClose =
      ...
      (event.relatedTarget &&
        !event.relatedTarget.classList.contains("disclosure-list__option"));
...
class GridList extends HTMLElement {
  constructor() {
    ...
    if (FoxTheme.config.motionReduced || this.hasAttribute("motion-reduced"))
      return;

    this.hideGridItems();
    FoxTheme.Motion.inView(this, this.showGridItems.bind(this), {
...
class AnnouncementBar extends HTMLElement {
  constructor() {
    ...
    FoxTheme.Motion.inView(this, this.initializeCarousel.bind(this), {
  ...
  get getSlideItems() {
    return (this._cachedSlideItems =
      this._cachedSlideItems ||
      Array.from(this.announcementItemsWrapper.children));
  }
  ...
  getAutoplaySpeed() {
    return this.hasAttribute("autoplay")
      ? parseInt(this.getAttribute("autoplay-speed")) * 1000
      : 5000;
  }
    ...
    if (this.getSlideItems.length > 1) {
      this.carousel = new FoxTheme.Carousel(
        this,
        {
          ...
          autoplay: this.isAutoplayEnabled()
            ? { delay: this.getAutoplaySpeed(), pauseOnMouseEnter: true }
            : false,
        },
      ...
      if (Shopify.designMode) {
        this.addEventListener("shopify:block:select", (event) => {
          this.carousel.slider.slideToLoop(
            this.getSlideItems.indexOf(event.target)
          );
        });
      }
    }
...
class ProductBundleVariantSelector extends HTMLElement {
  ...
  handleVariantChange(event) {
    ...
    const compareAtPrice = parseInt(
      selectedVariant.dataset.compare_at_price || 0
    );
    ...
    const compareAtPriceVaries =
      variantSelect.dataset.compare_at_price_varies === "true";
    ...
    regularPriceElement.querySelector(
      ".f-price-item--regular"
    ).innerHTML = `${FoxTheme.Currency.formatMoney(
      price,
    ...
    salePriceElement.querySelector(
      ".f-price-item--regular"
    ).innerHTML = `<s>${FoxTheme.Currency.formatMoney(
      compareAtPrice,
      FoxTheme.settings.moneyFormat
    )}</s>`;
    salePriceElement.querySelector(
...
class ProductsBundle extends HTMLElement {
  ...
  handleButtonClick(event) {
    ...
    if (
      document.body.classList.contains("cart-template") ||
    ...
    fetch(`${FoxTheme.routes.cart_add_url}`, {
      ...
      .then(async (parsedState) => {
        ...
        if (parsedState.status === 422) {
          ...
          this.cartDrawerElement &&
            this.cartDrawerElement.show(this.lastClickedElement);
        }
      })
      .catch((e) => {
        console.error(e);
...
class MotionElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    FoxTheme.Motion.inView(
      this,
      async () => {
        if (!this.isInstant && this.media)
          await FoxTheme.utils.imageReady(this.media);
        this.initialize();
      },
      { margin: "0px 0px -50px 0px" }
    );
...
class TabsComponent extends HTMLElement {
  ...
  getHeaderHeight() {
    ...
    if (this.headerEl) {
      ...
      if (this.headerEl.isAlwaysSticky) {
        ...
        const isSticky = document.body.classList.contains(
          this.headerEl.classes.pinned
        );
        ...
        if (
          isSticky &&
          window.scrollY >= this.offsetTop - this.scrollOffset - height
        ) {
          headerHeight = height;
        }
      }
  ...
  async transition(fromPanel, toPanel) {
    ...
    toPanel.querySelector("grid-list") &&
      toPanel.querySelector("grid-list").showGridItems();
  }
}
...
class Parallax extends HTMLElement {
  ...
  get parallaxMediaElements() {
    return Array.from(
      this.querySelectorAll("img, video, iframe, svg, video-element")
    );
  }
...
class ProductForm extends HTMLFormElement {
  constructor() {
    ...
    this.hideCartDrawer = this.dataset.hideCartDrawer === "true";
  ...
  get submitButtonElement() {
    return (this._submitButtonElement =
      this._submitButtonElement || this.querySelector('[type="submit"]'));
  }
  ...
  handleFormSubmit = (event) => {
    if (
      document.body.classList.contains("cart-template") ||
      FoxTheme.settings.cartType === "page"
    )
      return;
